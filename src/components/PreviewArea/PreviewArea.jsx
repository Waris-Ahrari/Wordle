...
const PreviewArea = () => {
  ...
  const { randomWord, isWordValid } = useContext(WordContext)
  ...
  const handleEnter = () => {
    if (currentCol >= n) {
      const currentRowItem = prevGrid[currentRow];
      const currentWord = currentRowItem.map(l => l.Letter).join('')
      if (isWordValid(currentWord)) {
        const newGrid = validate()
